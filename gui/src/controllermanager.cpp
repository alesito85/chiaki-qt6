	...
	if(SDL_GameControllerHasSensor(controller, SDL_SENSOR_ACCEL) && SDL_GameControllerHasSensor(controller, SDL_SENSOR_GYRO))
	{
		...
		if (state.l2_state != 0) {
			if (gyro_data[1] < -0.02f || gyro_data[1] > 0.02f) {
				state.right_x = (gyro_data[1] < 0 ? MAX(gyro_data[1], -2.0f) : MIN(gyro_data[1], 2.0f)) * 32767 * -1;
			}
			if (gyro_data[0] < -0.02f || gyro_data[0] > 0.02f) {
				state.right_y = (gyro_data[0] < 0 ? MAX(gyro_data[0], -2.0f) : MIN(gyro_data[0], 2.0f)) * 32767 * -1;
			}
		}
